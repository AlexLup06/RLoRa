...
W LoRaAnalogModel::computeReceptionPower(const IRadio *receiverRadio, const ITransmission *transmission, const IArrival *arrival) const
{
    ...
    W rxPower=transmissionPower * std::min(1.0, transmitterAntennaGain * receiverAntennaGain * pathLoss * obstacleLoss);

    EV<<"Transmitter Antenna Gain: "<<transmitterAntennaGain<<" W or: "<<math::mW2dBmW(transmitterAntennaGain)<<" dBi"<<endl;
    EV<<"Path Loss: "<<pathLoss <<" W or: "<<math::mW2dBmW(pathLoss)<<" dBm"<<endl;
    EV<<"Obstacle Loss: "<<obstacleLoss<<" or " << math::mW2dBmW(obstacleLoss)<<" dBm"<<endl;
    EV<<"Transmission Power: "<<transmissionPower.get()<<" or " << math::mW2dBmW(transmissionPower.get()*1000)<<" dBm"<<endl;
    EV<<"Reception Power: "<<rxPower<<" or " << math::mW2dBmW(rxPower.get()*1000)<<" dBm"<<endl;
