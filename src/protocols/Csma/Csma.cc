...
void Csma::initialize(int stage)
{
    ...
    if (stage == INITSTAGE_LOCAL) {
        ...
        fsm.setName("Csma State Machine");
    }
...
void Csma::configureNetworkInterface()
{
...
void Csma::encapsulate(Packet *msg)
{
...
void Csma::decapsulate(Packet *frame)
{
...
bool Csma::isInvalidBackoffPeriod()
{
    return backoffPeriod == -1;
}

void Csma::generateBackoffPeriod()
{
 ...
 * Helper functions.
 */
void Csma::finishCurrentTransmission()
{
    deleteCurrentTxFrame();
    resetTransmissionVariables();
}

Packet* Csma::getCurrentTransmission()
{
    ASSERT(currentTxFrame != nullptr);
    return currentTxFrame;
}

void Csma::resetTransmissionVariables()
{
    backoffPeriod = -1;
}

void Csma::emitPacketDropSignal(Packet *frame, PacketDropReason reason, int limit)
{
...
bool Csma::isMediumFree()
{
    return radio->getReceptionState() == IRadio::RECEPTION_STATE_IDLE;
}

bool Csma::isReceiving()
{
    return radio->getReceptionState() == IRadio::RECEPTION_STATE_RECEIVING;
}

void Csma::handleStopOperation(LifecycleOperation *operation)
{
    MacProtocolBase::handleStopOperation(operation);
    resetTransmissionVariables();
}

void Csma::handleCrashOperation(LifecycleOperation *operation)
{
    MacProtocolBase::handleCrashOperation(operation);
    resetTransmissionVariables();
}

void Csma::processUpperPacket()
{
    Packet *packet = dequeuePacket();
    handleUpperPacket(packet);
}

queueing::IPassivePacketSource* Csma::getProvider(cGate *gate)
{
    return (gate->getId() == upperLayerInGateId) ? txQueue.get() : nullptr;
}

void Csma::handleCanPullPacketChanged(cGate *gate)
{
...
void Csma::handlePullPacketProcessed(Packet *packet, cGate *gate, bool successful)
{
    Enter_Method("handlePullPacketProcessed");
    throw cRuntimeError("Not supported callback");
}

MacAddress Csma::getAddress()
{
    return address;
}

void Csma::createBroadcastPacket(int payloadSize, int missionId, int source, bool retransmit)
{
...
void Csma::announceNodeId(int respond)
{
...
void Csma::retransmitPacket(FragmentedPacket fragmentedPacket)
{
