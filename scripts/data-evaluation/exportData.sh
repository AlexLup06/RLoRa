...
for file in $SOURCE_DIR*.vec; do
    ...
    if [[ "$file" =~ mac([A-Za-z0-9]+)-maxX([0-9]+)m-ttnm([0-9.]+)s-numberNodes([0-9]+)-m([A-Za-z]+)-rep([0-9]+)\.vec ]]; then
        ...
        opp_scavetool export -f 'name=~"missionIdRtsSent:vector" OR name=~"missionIdFragmentSent:vector" OR name=~"receivedMissionId:vector"' -o $rlora_root/data/${protocol}/${range}/missionId-${count}.json -F JSON ${SOURCE_DIR}mac${protocol}-maxX${range}-ttnm${interval}-numberNodes${nodes}-m${mobility}-rep${rep}.vec

        opp_scavetool export -f 'name=~"timeOfLastTrajectory:vector"' -o $rlora_root/data/${protocol}/${range}/timeOfLastTrajectory-${count}.json -F JSON ${SOURCE_DIR}mac${protocol}-maxX${range}-ttnm${interval}-numberNodes${nodes}-m${mobility}-rep${rep}.vec

        opp_scavetool export -f 'name=~"receivedId:vector"' -o $rlora_root/data/${protocol}/${range}/idReceived-${count}.json -F JSON ${SOURCE_DIR}mac${protocol}-maxX${range}-ttnm${interval}-numberNodes${nodes}-m${mobility}-rep${rep}.vec
